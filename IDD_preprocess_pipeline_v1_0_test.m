function IDD_preprocess_pipeline_v1_0_test(datadir)
% ************************************************************************
% This code is modified from the Maryland Analysis of Developmental EEG
...
% Yan - add folder for ERP plots
if exist([output_location filesep 'processed_ERP_plots'], 'dir') == 0
    mkdir([output_location filesep 'processed_ERP_plots'])
end
...
ICs_removed_HEM=[]; % Yan - add labels of artifacted ICs 
ICs_removed_VEM=[];
ICs_removed_EB=[];
ICs_removed_GD=[];
...
epochs_before_artifact_rejection_by_condition=[]; % Yan - add epoch numbers by conditions
epochs_after_artifact_rejection_by_condition=[];
task_events_makers_all=[];
...
for subject=1:length(datafile_names)
    ...
    %EEG=pop_readegi([rawdata_location filesep datafile_names{subject}],[],[],'auto');
...
%     EEG = eeg_checkset( EEG );
%     EEG = pop_firws(EEG, 'fcutoff', high_cutoff, 'ftype', 'highpass', 'wtype', 'hamming', 'forder', hp_fl_order, 'minphase', 0, 'plotfresp',1);
%     EEG = eeg_checkset( EEG );
    ...
    if all_chan_bad_FAST==1
        ...
        ICs_removed_HEM{subject}='None'; % Yan - add removed IC labels
        ICs_removed_VEM{subject}='None';
        ICs_removed_EB{subject}='None';
        ICs_removed_GD{subject}='None';
        total_epochs_before_artifact_rejection(subject)=0;
        total_epochs_after_artifact_rejection(subject)=0;
        epochs_before_artifact_rejection_by_condition{subject}=0; % Yan - add epoch numbers by conditions
        epochs_after_artifact_rejection_by_condition{subject}=0;
   ...
   % If channels were removed from copied dataset during preparation of ica, then remove
    ...
    % Yan - save the label of the artifact ICs
    % horiz: HEM (horizontal Eye Movement) ICs
    % vert: VEM (Vertical Eye Movement) ICs
    % blink:EB (Eye blink) ICs
    % disc: GD (Generic discontinuities) ICS
    if save_interim_result==1
        [badICs,horiz,vert,blink,disc,~] = adjusted_ADJUST(EEG_copy, [[output_location filesep 'ica_data' filesep] strrep(datafile_names{subject}, ext, '_adjust_report')]);
    else
        [badICs,horiz,vert,blink,disc,~] = adjusted_ADJUST(EEG_copy, [[output_location filesep 'processed_data' filesep] strrep(datafile_names{subject}, ext, '_adjust_report')]);
    end
    close all;
       
    %[badICs,horiz,vert,blink,disc,~] = ADJUST(EEG_copy, [[output_location filesep 'ica_data' filesep] strrep(datafile_names{subject}, ext, '_adjust_report')]);
    
    % Yan - save the artifact label that's in badICs
    horiz = intersect(horiz,badICs);
    vert = intersect(vert,badICs);
    blink = intersect(blink,badICs);
    disc = intersect(disc,badICs);
    
    % remove the first 5 ICs in HEM, VEM and EM, ignor the GD -- % Yan
    if length(horiz) > 5
        horiz = horiz(1:5);
    end
    ...
    if numel(badICs)==0
        ...
        % Yan - add label for each IC
        ICs_removed_HEM{subject}=num2str(double(horiz));
        ICs_removed_VEM{subject}=num2str(double(vert));
        ICs_removed_EB{subject}=num2str(double(blink));
        ICs_removed_GD{subject}=num2str(double(disc));        
    end
    ...
    % Yan - split the epochs by conditions
    events_before = extractfield(EEG.epoch,'eventcode');
    ...
    task_events_markers_all{subject} = [sprintf('%s ',task_event_markers{1:end-1}),task_event_markers{end}]; % all subjects have the same event markers
    ...
    if all_bad_epochs==1
        ...
        % split the rejected epochs by conditions - Yan
        events_after = extractfield(EEG.epoch,'eventcode');
        events_after_labels = cell(1, length(EEG.epoch));
        for i = 1:length(EEG.epoch)
            %ind = ismember(events_after{i},task_event_markers);
            %events_after_labels{i} = task_event_markers{ind};
          
            events = events_after{i};
            ind = find(ismember(events,task_event_markers));
            if iscell(events)
            events_after_labels{i} = events{ind};
            else  events_after_labels{i} = events;
            end
    
        end
        
        epochs_after = cellfun(@(x) sum(ismember(events_after_labels,x)),task_event_markers,'un',0);
        epochs_after_artifact_rejection_by_condition{subject}= num2str(cell2mat(epochs_after));
...
%     end
    ...
    if rerefer_data==1
        if iscell(reref)==1
            ...
            EEG = eeg_checkset(EEG); 
            EEG = pop_reref(EEG, reref); % use average ref - Yan
        end
    end
    %% STEP 15: Interpolate deleted channels
    if interp_channels==1
    ...
    %EEG = pop_loadset('filename', strrep(datafile_names{subject}, ext, '_processed_data.set'),'filepath', [output_location filesep 'processed_data' filesep ]);
    
    
    %% Yan - Step 17: Plot ERPs at user specified channels
    if plot_ERPs==1
    figure; 
    pop_timtopo(EEG, [EEG.xmin*1000 EEG.xmax*1000], [NaN], ...
        ['ERP data and scalp maps of ' strrep(datafile_names{subject}, ext, '_processed_data')],...
        'plotchans',chans_to_plot);
    saveas(gcf,[output_location filesep 'processed_ERP_plots' filesep strrep(datafile_names{subject}, ext, '_processed_data_ERPs.png')])
    close all;
    end
    
    %% Yan - Step 18: Plot ERPs by stimulus
    if plot_ERPs_by_condition == 1
        tmp = find(cell2mat(epochs_after));
        if isempty(find(tmp, 1)) % no usable epochs
            print("no usable epochs");
        elseif find(tmp(1))&&find(tmp(2))  % both stimulus have non-zero epochs
         % select epochs for different stimulus
         %location = [output_location filesep 'processed_data'];
         %EEG = pop_loadset('filename',file,'filepath',location);
            EEG_1 = pop_selectevent( EEG, 'type',task_event_markers(1),'deleteevents','on','deleteepochs','on','invertepochs','off');
            EEG_1 = pop_saveset( EEG_1, 'filename', strrep(datafile_names{subject}, '.mff',append('_',task_event_markers{1},'.set')),'filepath', output_location); 
            

            EEG_2 = pop_selectevent( EEG, 'type',task_event_markers(2),'deleteevents','on','deleteepochs','on','invertepochs','off');
            EEG_2 = pop_saveset( EEG_2, 'filename', strrep(datafile_names{subject}, '.mff',append('_',task_event_markers{2},'.set')),'filepath', output_location); 
            
            % take the mean across cluster and trials 
            EEG_task_1 = mean(EEG_1.data(chans_to_plot,:,:),[1,3]);   
            EEG_task_2 = mean(EEG_2.data(chans_to_plot,:,:),[1,3]);

            % plot togeter
            figure;
            hold on
            a1 = plot(EEG.times,EEG_task_1); 
            M1 = string(task_event_markers{1});
            a2 = plot(EEG.times,EEG_task_2); 
            M2 = string(task_event_markers{2});
            legend([a1,a2],[M1,M2]);
            xlim([-100 900]);
            xlabel('ms')
            ylabel('uV')
            hold off
            saveas(gcf,[output_location filesep 'processed_ERP_plots' filesep strrep(datafile_names{subject}, ext, '_processed_data_ERPs_by_condition.png')])
            close all;
        elseif find(tmp(1))==0 % no repeat, only single
            ...
            figure;
            %hold on
            a2 = plot(EEG.times,EEG_task_2); M2 = string(task_event_markers{2});
            legend(a2,M2);
            %hold off
            saveas(gcf,[output_location filesep 'processed_ERP_plots' filesep strrep(datafile_names{subject}, ext, '_processed_data_ERPs_by_condition.png')])
            close all;
        elseif find(tmp(2))==0 % no single, only repeat
            EEG_1 = pop_selectevent( EEG, 'type',task_event_markers(1),'deleteevents','on','deleteepochs','on','invertepochs','off');
            EEG_1 = pop_saveset( EEG_1, 'filename', strrep(datafile_names{subject}, '.mff',append('_',task_event_markers{1},'.set')),'filepath', output_location); 
            ...
            figure;
            a1 = plot(EEG.times,EEG_task_1); M1 = string(task_event_markers{1});
            legend(a1,M1);
...
report_table=table(datafile_names', reference_used_for_faster', faster_bad_channels', ica_preparation_bad_channels', length_ica_data', ...
    total_ICs', ICs_removed', ICs_removed_HEM', ICs_removed_VEM',ICs_removed_EB', ICs_removed_GD',total_epochs_before_artifact_rejection', ...
    total_epochs_after_artifact_rejection',total_channels_interpolated',...
    task_events_markers_all',epochs_before_artifact_rejection_by_condition',epochs_after_artifact_rejection_by_condition'); 
...
report_table.Properties.VariableNames={'datafile_names', 'reference_used_for_faster', 'faster_bad_channels', ...
    'ica_preparation_bad_channels', 'length_ica_data', 'total_ICs', 'ICs_removed', 'ICs_removed_HEM',...
    'ICs_removed_VEM','ICs_removed_EB','ICs_removed_GD','total_epochs_before_artifact_rejection', ...
    'total_epochs_after_artifact_rejection', 'total_channels_interpolated','task_event_markers',...
